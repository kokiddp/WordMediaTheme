...
class WMT_Builder_Module_WM_SocialButtons extends ET_Builder_Module {
	function init() {
		$this->name       = esc_html__( 'WordMedia Social Buttons', 'wordmedia' );
		$this->slug       = 'et_pb_wm_socialbuttons';
		...
		$this->main_css_element = '%%order_class%%.et_pb_wm_socialbuttons';
	...
	function get_fields() {
		...
		while ($query->have_posts()) {
		    $query->the_post();
		    $post_id = get_the_ID();
		    $socialbuttons[$post_id] = get_the_title($post_id);
		}
		wp_reset_query();

		$fields = array(
			'id' => array(
				'label'           => esc_html__( 'WordMedia Social Buttons', 'wordmedia' ),
				'type'            => 'select',
				'option_category' => 'basic_option',
				'options'         => $socialbuttons,
				'description'     => esc_html__( 'Pick the social buttons', 'wordmedia' ),
				'is_fb_content'   => true,
			),
			'disabled_on' => array(
	...
	function shortcode_callback( $atts, $content = null, $function_name ) {
		...
		ob_start();
		echo do_shortcode('[wordmedia_socialbuttons id="' . $id . '"]');
		$content = ob_get_contents();
		ob_end_clean();

		$output = sprintf(
			'<div%1$s class="et_pb_module et_pb_wm_socialbuttons%2$s">
				%3$s
			</div>',
			$module_id,
			$module_class,
...
new WMT_Builder_Module_WM_SocialButtons;