...
class WMT_Builder_Module_WM_Footer extends ET_Builder_Module {
	function init() {
		$this->name       = esc_html__( 'WordMedia Footer', 'wordmedia' );
		$this->slug       = 'et_pb_wm_footer';
		...
		$this->main_css_element = '%%order_class%%.et_pb_wm_footer';
	...
	function get_fields() {
		...
		while ($query->have_posts()) {
		    $query->the_post();
		    $post_id = get_the_ID();
		    $footers[$post_id] = get_the_title($post_id);
		}
		wp_reset_query();

		$fields = array(
			'id' => array(
				'label'           => esc_html__( 'WordMedia Footer', 'wordmedia' ),
				'type'            => 'select',
				'option_category' => 'basic_option',
				'options'         => $footers,
				'description'     => esc_html__( 'Pick the Footer', 'wordmedia' ),
				'is_fb_content'   => true,
			),
			'disabled_on' => array(
	...
	function shortcode_callback( $atts, $content = null, $function_name ) {
		...
		ob_start();
		echo do_shortcode('[wordmedia_footer id="' . $id . '"]');
		$content = ob_get_contents();
		ob_end_clean();

		$output = sprintf(
			'<div%1$s class="et_pb_module et_pb_wm_footer%2$s">
				%3$s
			</div>',
			$module_id,
			$module_class,
...
new WMT_Builder_Module_WM_Footer;