...
class WMT_Builder_Module_WM_Map extends ET_Builder_Module {
	function init() {
		$this->name       = esc_html__( 'WordMedia Map', 'wordmedia' );
		$this->slug       = 'et_pb_wm_map';
		...
		$this->main_css_element = '%%order_class%%.et_pb_wm_map';
	...
	function get_fields() {
		...
		while ($query->have_posts()) {
		    $query->the_post();
		    $post_id = get_the_ID();
		    $maps[$post_id] = get_the_title($post_id);
		}
		wp_reset_query();

		$fields = array(
			'id' => array(
				'label'           => esc_html__( 'WordMedia Map', 'wordmedia' ),
				'type'            => 'select',
				'option_category' => 'basic_option',
				'options'         => $maps,
				'description'     => esc_html__( 'Pick the map', 'wordmedia' ),
				'is_fb_content'   => true,
			),
			'disabled_on' => array(
	...
	function shortcode_callback( $atts, $content = null, $function_name ) {
		...
		ob_start();
		echo do_shortcode('[wordmedia_map id="' . $id . '"]');
		$content = ob_get_contents();
		ob_end_clean();

		$output = sprintf(
			'<div%1$s class="et_pb_module et_pb_wm_map%2$s">
				%3$s
			</div>',
			$module_id,
			$module_class,
