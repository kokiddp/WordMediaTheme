...
class WMT_Builder_Module_WM_Evidence extends ET_Builder_Module {
	function init() {
		$this->name       = esc_html__( 'WordMedia Evidence', 'wordmedia' );
		$this->slug       = 'et_pb_wm_evidence';
		...
		$this->main_css_element = '%%order_class%%.et_pb_wm_evidence';
	...
	function get_fields() {
		...
		while ($query->have_posts()) {
		    $query->the_post();
		    $post_id = get_the_ID();
		    $evidences[$post_id] = get_the_title($post_id);
		}
		wp_reset_query();

		$fields = array(
			'id' => array(
				'label'           => esc_html__( 'WordMedia Evidence', 'wordmedia' ),
				'type'            => 'select',
				'option_category' => 'basic_option',
				'options'         => $evidences,
				'description'     => esc_html__( 'Pick the evidence', 'wordmedia' ),
				'is_fb_content'   => true,
			),
			'disabled_on' => array(
	...
	function shortcode_callback( $atts, $content = null, $function_name ) {
		...
		ob_start();
		echo do_shortcode('[wordmedia_evidences id="' . $id . '"]');
		$content = ob_get_contents();
		ob_end_clean();

		$output = sprintf(
			'<div%1$s class="et_pb_module et_pb_wm_evidence%2$s">
				%3$s
			</div>',
			$module_id,
			$module_class,
...
new WMT_Builder_Module_WM_Evidence;